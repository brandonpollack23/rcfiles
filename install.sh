...
function setup_home_dir() {
  ...
  ln -sfn "$RCFILES_DIR/.iex.exs" "$HOME/.iex.exs"
...
if [[ -n "$HYPRLAND_INSTANCE_SIGNATURE" ]]; then
  "$RCFILES_DIR/.config/hypr/scripts/system.sh" install-plugins ||
    echo "Hyprland plugin install failed; retry with 'Install/update Hyprland plugins' in SUPER+SHIFT+R" >&2
else
  echo "Not in Hyprland: it installs its plugins when it next starts" >&2
fi
